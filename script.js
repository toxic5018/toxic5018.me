...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchAndDisplayVersion(retryCount = 3) { // Added retryCount with default
        console.log(`Attempting to fetch version.xml (Attempt ${4 - retryCount})...`); // Log fetch attempt

        fetch('version.xml')
            .then(response => {
                 console.log(`version.xml fetch response status: ${response.status}`); // Log response status
                 if (!response.ok) {
                     console.error(`HTTP error fetching version.xml! Status: ${response.status}`); // Error on bad HTTP status
                     throw new Error(`HTTP error! status: ${response.status}`);
                 }
            ...
            .then(str => {
                ...
                const errorNode = xmlDoc.querySelector('parsererror');
                if (errorNode) {
                     console.error('Error parsing version.xml (parsererror element):', errorNode.textContent); // Error on parse error
                     throw new Error('version.xml parsing error reported in parsererror element.');
                }
            ...
            .then(data => {
                ...
                if (versionElement && versionElement.textContent && versionInfoElement) {
                    versionInfoElement.textContent = `Version: ${versionElement.textContent}`; // Update text
                     console.log(`Version updated successfully: ${versionElement.textContent}`); // Log success
                } else {
                    console.warn('Version element not found in version.xml or version info HTML element not found.'); // Warn if elements missing
            ...
            .catch(error => {
                console.error(`Error fetching or parsing version.xml (Attempt ${4 - retryCount}):`, error); // Log error with attempt number
    ...
    const clickCounter = document.querySelector('.level-bar-container .click-counter');
    const levelBarFill = document.querySelector('.level-bar-container .level-bar-fill');
    ...
    function loadLevelProgress() {
        ...
        try {
            ...
            if (savedProgress) {
                ...
                if (typeof progress.level === 'number' && typeof progress.clicks === 'number' && progress.clicks >= 0 && progress.clicks <= clicksToLevelUp) {
                     ...
                     // Update UI immediately with loaded progress
                       if (levelText && clickCounter && levelBarFill) {
                           ...
                           console.log('Level progress loaded and applied from localStorage.', progress); // Log load and apply
                           ...
                           if (clicksInCurrentLevel > 0) {
                               ...
                               console.log('Level bar made visible on load due to existing progress.'); // Log visibility
                               resetLevelBarTimer(); // Start the hide timer
                           }
                       } else {
                           console.warn('Level bar UI elements not found during loadLevelProgress (UI update failed).'); // Warn if UI missing
                       }

                } else {
                    console.warn('Invalid level progress data found in localStorage, starting from Level 0.', savedProgress); // Warn about invalid data
                ...
                console.log('No saved level progress found in localStorage, starting from Level 0.'); // Log no data
            ...
            console.error('Error loading or parsing level progress from localStorage:', e); // Error on load/parse failure
    ...
    if (profilePicture && levelBarContainer && levelText && clickCounter && levelBarFill) {
    ...
    const systemDefaultCheckbox = document.getElementById('systemDefaultCheckbox'); // New system default checkbox
    const darkModeCheckbox = document.getElementById('darkModeCheckbox');
    const backgroundMotionCheckbox = document.getElementById('backgroundMotionCheckbox');
    const visualEffectsCheckbox = document.getElementById('visualEffectsCheckbox');
    const body = document.body;
    // const settingItems = document.querySelectorAll('.setting-item label[data-description]'); // No longer needed, combined with has-tooltip
    ...
    function handleSystemThemeChange(event) {
        if (systemDefaultCheckbox && systemDefaultCheckbox.checked) { // Only react if system default is active
            const newTheme = event.matches ? 'dark' : 'light';
            applyTheme(newTheme);
            console.log(`System theme changed to ${newTheme}. Website theme updated.`);
        }
    }
     ...
     if (systemDefaultCheckbox && darkModeCheckbox) {
         systemDefaultCheckbox.addEventListener('change', () => {
             ...
             if (systemDefaultCheckbox.checked) {
                 ...
                 console.log('System default theme enabled. Dark Mode option disabled. Preference saved.'); // Log state change
             } else {
                 ...
                 } else { // 'disabled' or no preference saved
                 ...
                 console.log('System default theme disabled. Dark Mode option enabled. Reverted to saved preference.'); // Log state change
             }
         });
         console.log('System Default checkbox event listener added.'); // Log listener added
     } else {
         console.warn('System Default or Dark Mode checkbox element not found. System default functionality may not work.'); // Warn if elements missing
     }
        ...
        darkModeCheckbox.addEventListener('change', () => {
            ...
            if (!systemDefaultCheckbox.checked) {
                ...
                if (darkModeCheckbox.checked) {
                    ...
                    console.log('Dark mode disabled (manual). Preference saved to localStorage.'); // Log disabled and saved'); // Log disabled and saved
                }
            } else {
                 // If system default is checked, uncheck the dark mode box (it should be disabled anyway)
                 darkModeCheckbox.checked = false;
                 console.log('Attempted to change Dark Mode checkbox while System Default is enabled. Action blocked.'); // Log blocked action
            }
        });
         console.log('Dark Mode checkbox event listener added.'); // Log listener added
    } else {
         console.warn('Dark Mode or System Default checkbox element not found. Dark mode functionality may not work as intended.'); // Warn if elements missing'); // Warn if elements missing'); // Warn if elements missing'); // Warn if elements missing
    }
        ...
        backgroundMotionCheckbox.addEventListener('change', () => {
            ...
            if (backgroundMotionCheckbox.checked) {
                ...
                localStorage.setItem('backgroundMotion', 'enabled'); // Save preference
                 console.log('Background motion enabled. Preference saved to localStorage.'); // Log enabled and saved'); // Log enabled and saved
            } else {
                body.classList.remove('background-motion-enabled');
                localStorage.setItem('backgroundMotion', 'disabled'); // Save preference
                 console.log('Background motion disabled. Preference saved to localStorage.'); // Log disabled and saved'); // Log disabled and saved
            }
             // Add logging to see the class state after change
            console.log('Body classes after background motion change:', body.classList.value);
        });
        ...
        visualEffectsCheckbox.addEventListener('change', () => {
            ...
            if (!visualEffectsCheckbox.checked) {
                ...
                localStorage.setItem('visualEffects', 'disabled'); // Save preference
                 console.log('Visual effects disabled. Preference saved to localStorage.'); // Log disabled and saved'); // Log disabled and saved
            } else {
                body.classList.remove('no-visual-effects');
                localStorage.setItem('visualEffects', 'enabled'); // Save preference
                 console.log('Visual effects enabled. Preference saved to localStorage.'); // Log enabled and saved'); // Log enabled and saved
            }
        });
    ...
    // Get references to all elements that should show a tooltip on hover
    // This now includes settings labels AND header elements with the 'has-tooltip' class
    const elementsWithTooltip = document.querySelectorAll('.setting-item label[data-description], .has-tooltip[data-description]'); // Updated selector

    // const tooltip = document.querySelector('.setting-description-tooltip'); // Tooltip element is already referenced earlier

    if (elementsWithTooltip.length > 0 && tooltip) {
        elementsWithTooltip.forEach(item => {
            item.addEventListener('mouseover', (event) => {
                ...
                if (description) {
                    ...
                    // console.log(`Showing tooltip for: "${description}"`); // Log showing tooltip - uncomment for debugging
                }
            });
            ...
            item.addEventListener('mousemove', (event) => {
                 ...
                 // Adjust tooltip position based on its own size to appear above the cursor
            ...
            item.addEventListener('mouseout', () => {
                ...
                // console.log('Hiding tooltip.'); // Log hiding tooltip - uncomment for debugging
            });
        });
         console.log('Hover/mousemove listeners added for tooltip elements.'); // Log listeners added
    } else {
         console.warn('No elements with tooltip descriptions or tooltip element not found.'); // Warn if elements missing
    }
    ...
    console.log('Attempting to load settings (System Default, Dark Mode, Background Motion, Visual Effects) from localStorage...'); // Log load attempt
    ...
    if (systemDefaultCheckbox && darkModeCheckbox) {
        if (savedUseSystemDefault === 'enabled') {
            ...
            if (savedDarkMode === 'enabled') {
                 applyTheme('dark');
                 darkModeCheckbox.checked = true;
                 console.log('Manual dark mode preference loaded and applied from localStorage.');
             } else { // 'disabled' or no preference saved for dark mode
                 applyTheme('light'); // Default to light if no explicit dark mode preference
                 darkModeCheckbox.checked = false;
                 console.log('Manual dark mode preference disabled or not found, defaulting to light.');
             }
        }
    } else {
         console.warn('System Default or Dark Mode checkbox element not found during settings load.');
    }
    ...
    // Load and apply Background Motion setting
     if (backgroundMotionCheckbox) {
         if (savedBackgroundMotion === 'enabled') {
             ...
             console.log('No background motion preference found in localStorage, using default (disabled).');
             backgroundMotionCheckbox.checked = false;
             body.classList.remove('background-motion-enabled');
         }
     } else {
          console.warn('Background Motion checkbox element not found during settings load.');
     }
     ...
     if (visualEffectsCheckbox) {
         ...
         if (savedVisualEffects === 'disabled') {
             ...
             body.classList.add('no-visual-effects');
             console.log('Visual effects disabled loaded and applied from localStorage.');
          ...
          console.warn('Visual Effects checkbox element not found during settings load.');
     }
